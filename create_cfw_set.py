...
if args.poison_type == 'dynamic':
    ...
    if args.dataset == 'gtsrb':
        ...
        train_set = CIFAR20(os.path.join(data_dir, 'cifar100'), train=True,
                                      download=True, transform=data_transform)
        img_size = 32
        num_classes = 20
...
poison_set_img_dir = os.path.join(poison_set_dir, 'data') #non_wm_data
...
if args.poison_type in ['badnet', 'blend', 'clean_label',
                        ...
                        'SIG', 'TaCT', 'WaNet', 'none',
                        'trojan', 'entangled_watermark',
                        'meadefender', 'ood_class', 'sub_ood_class', 'UAE']:
    ...
    if args.poison_type == 'badnet':
        ...
        from poison_tool_box import entangled_watermark
        poison_set_img_dir = os.path.join(poison_set_dir, 'data')
        if not os.path.exists(poison_set_img_dir):
            os.makedirs(poison_set_img_dir)

        poison_generator = entangled_watermark.watermark_generator(args=args, img_size=img_size, dataset=train_set,
                                                path=poison_set_img_dir, target_class=config.target_class[args.dataset])
    elif args.poison_type == 'meadefender':
        from poison_tool_box import meadefender
        poison_generator = meadefender.watermark_generator(args=args, img_size=img_size, dataset=train_set,
                                                path=poison_set_img_dir, target_class=config.target_class[args.dataset])

    elif args.poison_type == 'UAE':
        from poison_tool_box import UAE
        poison_generator = UAE.watermark_generator(args=args, path=poison_set_img_dir)

    elif args.poison_type == 'ood_class':
        from poison_tool_box import ood_class
        poison_generator = ood_class.watermark_generator(args=args, path=poison_set_img_dir)

    elif args.poison_type == 'sub_ood_class':
        from poison_tool_box import ood_class

        poison_generator = ood_class.watermark_generator(args=args, path=poison_set_img_dir, target_class=0)
    ...
    if args.poison_type in ['entangled_watermark',  'meadefender', 'ood_class', 'sub_ood_class', 'UAE']:#all watermarks
        poison_indices = []
        label_set = poison_generator.generate_poisoned_training_set()
        print("label_set", label_set.shape)

    elif args.poison_type not in ['TaCT', 'WaNet', 'adaptive_blend', 'adaptive_patch']:

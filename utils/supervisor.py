...
def get_extract_model_name(args, extract='distill'):
    if args.no_aug:
        model_name = f'full_base_no_aug_seed={args.seed}.pt'
    else:
        model_name = f'full_base_aug_seed={args.seed}.pt'
    if extract is not None:
        model_name = f"extract_{extract}_{model_name}"
    return model_name

def get_model_name(args, cleanse=None):
    ...
    #     model_name = 'secured.pth.tar'
    elif 'mu_' in args.poison_type:
        model_name = '1-extract.pth'
        # model_name = '1-extract-9.pth'
        # model_name = 'best.pth'
    else:
        if args.no_aug:
            # if args.lipschiz:
            #     model_name = f'full_lipschiz_no_aug_seed={args.seed}.pt'
            # else:
            #     model_name = f'full_base_no_aug_seed={args.seed}.pt'
            model_name = f'full_base_no_aug_seed={args.seed}.pt'
        else:
            # if args.lipschiz:
            #     model_name = f'full_lipschiz_aug_seed={args.seed}.pt'
            # else:
            #     model_name = f'full_base_aug_seed={args.seed}.pt'
            model_name = f'full_base_aug_seed={args.seed}.pt'
    ...
    if cleanse is not None:
        model_name = f"cleansed_{cleanse}_{model_name}"
    return model_name

def get_model_dir(args, cleanse=None):
    if hasattr(args, 'model_path') and args.model_path is not None:
        return args.model_path
    else:
        return f"{get_poison_set_dir(args)}/{get_model_name(args, cleanse=cleanse)}"

def get_extract_model_dir(args, extract='distill'):
    return f"{get_poison_set_dir(args)}/{get_extract_model_name(args, extract=extract)}"
...
def get_poison_set_dir(args):
    ...
    if args.poison_type == 'blend' or args.poison_type == 'basic' or args.poison_type == 'clean_label':
        blend_alpha = '%.3f' % args.alpha
        poison_set_dir = 'watermark_output_file/%s/%s_%s_alpha=%s_trigger=%s' % (args.dataset, args.poison_type, ratio, blend_alpha, args.trigger)
    elif args.poison_type == 'adaptive' or args.poison_type == 'adaptive_blend':
        blend_alpha = '%.3f' % args.alpha
        cover_rate = '%.3f' % args.cover_rate
        poison_set_dir = 'watermark_output_file/%s/%s_%s_alpha=%s_cover=%s_trigger=%s' % (args.dataset, args.poison_type, ratio, blend_alpha, cover_rate, args.trigger)
    elif args.poison_type == 'adaptive_patch' or args.poison_type == 'TaCT' or args.poison_type == 'WaNet':
        cover_rate = '%.3f' % args.cover_rate
        poison_set_dir = 'watermark_output_file/%s/%s_%s_cover=%s' % (args.dataset, args.poison_type, ratio, cover_rate)
    elif args.poison_type == 'adaptive_patch' or args.poison_type == 'adaptive_physical':
        poison_set_dir = 'watermark_output_file/%s/%s_%s_trigger=%s' % (args.dataset, args.poison_type, ratio, args.trigger)
    else:
        poison_set_dir = 'watermark_output_file/%s/%s_%s' % (args.dataset, args.poison_type, ratio)

    if not (args.poison_type == 'meadefender' or args.poison_type == 'sslguard' or args.poison_type == 'entangedwm'
            or 'mu_' in args.poison_type or args.poison_type == 'ood_class' or args.poison_type == 'sub_ood_class'):
...
def get_poison_transform(poison_type, dataset_name, target_class, source_class=1, cover_classes=[5,7],
    ...
    if dataset_name in ['gtsrb', 'cifar10', 'cifar20', 'cifar100']:
        img_size = 32
    else:
        raise NotImplementedError('<Undefined> Dataset = %s' % dataset_name)

    if dataset_name == 'cifar10' or dataset_name == 'cifar20':
    ...
    if (poison_type in ['badnet', 'blend', 'clean_label',
                       ...
                       'SIG', 'TaCT', 'WaNet', 'none',
                       'trojan', 'meadefender', 'sslguard', 'entangled_watermark', 'MBW', 'UAE', 'ood_class']
            or ('mu_' in poison_type)
            or ('ood' in poison_type)):

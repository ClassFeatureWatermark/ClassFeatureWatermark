...
def dfme_estimate_gradient_objective(args, victim_model, clone_model, x, epsilon = 1e-7, m = 5, verb=False,
                                num_classes=10, device = "cpu", pre_x=False, dataset='cifar10'):
    ...
    with torch.no_grad():
        ...
        if pre_x:
            if dataset == 'speech_commands':
                evaluation_points = 40*args.G_activation(evaluation_points)-40  # Apply args.G_activation function
            else:

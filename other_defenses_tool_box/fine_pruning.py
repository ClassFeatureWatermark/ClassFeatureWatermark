...
class FP(BackdoorDefense):
    ...
    def detect(self):
        ...
        # self.ori_clean_acc, _ = test(self.model, test_loader=self.test_loader,
    ...
    def prune(self):
        ...
        # test(self.model, test_loader=self.test_loader, poison_test=True, poison_transform=self.poison_transform,
    ...
    def prune_step(self, mask: torch.Tensor, prune_num: int = 1):
        ...
        for _input, _label in self.valid_loader:
            ...
            _, _feats = self.model.forward(_input, feature=True)#return_hidden=True)
            _feats = _feats.abs()
            # if _feats.dim() > 2:

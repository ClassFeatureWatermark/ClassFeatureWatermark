...
class I_BAU(BackdoorDefense):
    def __init__(self, args):
        ...
        self.optim = 'SGD'#'Adam'
        self.lr = 0.001 #0.001, 0.1 for badnet #0.001
        self.ratio = 0.05
        self.n_rounds = 10#10
        # self.K = 5
    ...
    def detect(self):
        ...
        print("Prepare Patch Models")
        EmbbedNet = Embbed()
        EmbbedNet = EmbbedNet.cuda()
        # ReverseNet = U_Net(output_ch=6*self.num_classes)
        ReverseNet = U_Net(output_ch = 6 * self.num_classes)
        ReverseNet = ReverseNet.cuda()
        optimizer_rev = torch.optim.Adam(
            ReverseNet.parameters(), lr=self.lr_optimizer_for_t)  # TODO TriggerNet?
        ReverseNet.train()
        ...
        for round in range(self.n_rounds):
            ...
            for clean_round in range(5):
                for images, labels in self.clean_loader:
                    ...
                    per_logits = self.model(images + batch_pert)

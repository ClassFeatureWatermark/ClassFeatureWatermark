...
class ABL(BackdoorDefense):
    ...
    def isolate(self):
        ...
        """
        print('----------- Train isolated model -----------') #only on poison set?
    ...
    def train_step_isolation(self, train_loader,
                             model_ascent,
                             optimizer,
                             criterion,
                             epoch):
    ...
    def train_isolation(self):
        ...
        poison_set_dir, poisoned_set_loader, poison_indices, cover_indices = (
            unpack_poisoned_train_set(args, batch_size=self.batch_size_isolation,
                                      shuffle=True, data_transform=self.tf_compose_isolation))
        ...
        for epoch in range(0, self.isolation_epochs):
            ...
            # val_atk(self.args, model_ascent)
            test(model_ascent, test_loader=self.test_loader,
                 poison_test=True, poison_transform=self.poison_transform, num_classes=self.num_classes,
                 source_classes=self.source_classes, all_to_all=('all_to_all' in self.args.dataset))
    ...
    def train_unlearn(self):
        ...
        for epoch in range(0, self.unlearning_epochs):
            ...
            if epoch == 0:
                ...
                # val_atk(self.args, model_ascent)
                test(model_ascent, test_loader=self.test_loader, poison_test=True, poison_transform=self.poison_transform, num_classes=self.num_classes, source_classes=self.source_classes, all_to_all=('all_to_all' in self.args.dataset))
            self.train_step_unlearning(isolate_poisoned_data_loader,
                                       model_ascent, optimizer, criterion, epoch + 1)
